...
# =========================
async def send_email(*, from_address: str, to_address: str, subject: str, html: str) -> tuple[bool, str | None]:
    """Compatibility wrapper for services.auto_reply_engine.

    The legacy auto-reply engine calls `send_email(from_address, to_address, subject, html)`.
    In this project, the real sender is `send_email_via_account(account, ...)`.

    This wrapper finds the EmailAccount by its email (from_address) and sends via that account.
    """
    ...
    async with Session() as session:
        acc = (await session.execute(
            sa_select(EmailAccount).where(func.lower(EmailAccount.email) == from_address).limit(1)
        )).scalars().first()

    if not acc:
        return False, f"ACCOUNT_NOT_FOUND:{from_address}"
